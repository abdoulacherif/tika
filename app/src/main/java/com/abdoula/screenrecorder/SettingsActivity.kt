...
class SettingsActivity : AppCompatActivity() {
    ...
    private val bubblePositions =
        listOf("top_left", "top_right", "bottom_left", "bottom_right")
    private val bubblePositionLabels =
        listOf("Haut à gauche", "Haut à droite", "Bas à gauche", "Bas à droite")

    private val logoPickerLauncher =
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<LinearLayout>(R.id.resolutionRow)
            .setOnClickListener { showResolutionDialog() }

        findViewById<LinearLayout>(R.id.bitrateRow)
            .setOnClickListener { showBitrateDialog() }

        findViewById<LinearLayout>(R.id.frameRateRow)
            .setOnClickListener { showFrameRateDialog() }

        findViewById<LinearLayout>(R.id.countdownRow)
            .setOnClickListener { showCountdownDialog() }

        findViewById<LinearLayout>(R.id.bubblePositionRow)
            .setOnClickListener { showBubblePositionDialog() }

        findViewById<LinearLayout>(R.id.batteryRow)
            .setOnClickListener { requestBatteryExemption() }

        findViewById<LinearLayout>(R.id.showTapsRow)
            .setOnClickListener { openDeveloperOptions() }

        findViewById<LinearLayout>(R.id.backupFolderRow)
            .setOnClickListener {
                backupFolderLauncher.launch(null)
            }

        findViewById<android.widget.Button>(R.id.logoPickerButton)
            .setOnClickListener {
                logoPickerLauncher.launch(arrayOf("image/*"))
            }

        findViewById<android.widget.Button>(R.id.redeemCodeButton)
            .setOnClickListener {
        ...
        watermarkTextInput.setText(
            SettingsManager.getWatermarkText(this)
        )

        val hideBubbleCheck = findViewById<CheckBox>(R.id.hideBubbleCheck)
        hideBubbleCheck.isChecked =
            SettingsManager.isBubbleHiddenDuringRecording(this)
        ...
        popupCheck.isChecked =
            SettingsManager.isPostRecordingPopupEnabled(this)

        popupCheck.setOnCheckedChangeListener { _, checked ->
            if (!checked && !SettingsManager.isProUser(this)) {
                popupCheck.isChecked = true
                showProDialog(
                    "Masquer le résumé après l'enregistrement est réservé à la version Pro."
                )
            } else {
        ...
        remindersCheck.isChecked =
            SettingsManager.areRemindersEnabled(this)
    ...
    private fun setupPinRow() {
        ...
        findViewById<LinearLayout>(R.id.pinRow).setOnClickListener {
            if (SettingsManager.isPinEnabled(this)) {

                AlertDialog.Builder(this)
                    .setTitle("Désactiver le verrouillage ?")
                    .setPositiveButton("Désactiver") { _, _ ->
                        SettingsManager.disablePin(this)
                ...
                val input = EditText(this).apply {
                    hint = "Choisis un code à 4 chiffres"
                    inputType =
                ...
                AlertDialog.Builder(this)
                    ...
                    .setPositiveButton("Valider") { _, _ ->
                        ...
                        if (pin.length == 4) {
                            SettingsManager.setPin(this, pin)
    ...
    private fun startTrialOrShowStatus() {
        if (SettingsManager.isProUser(this) &&
            ...
            Toast.makeText(
                this,
                "Il te reste environ $hours h d'essai Pro",
                Toast.LENGTH_LONG
            ).show()
    ...
    private fun refreshTrialCard() {
        when {
            ...
            SettingsManager.hasUsedTrial(this) -> {
                ...
                trialSubtitle.text =
                    "Ton essai gratuit de 3 jours est terminé"
    ...
    private fun showProDialog(message: String) {
        AlertDialog.Builder(this)
            ...
            .setMessage(
                "$message Toutes les autres fonctionnalités restent gratuites et illimitées."
            )
    ...
    private fun requestBatteryExemption() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            try {
                ...
                Toast.makeText(
                    this,
                    "Impossible d'ouvrir ce réglage sur ce téléphone",
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
    ...
    private fun openDeveloperOptions() {
        try {
            ...
            Toast.makeText(
                this,
                "Active d'abord le mode développeur (7 taps sur le numéro de build dans À propos du téléphone)",
                Toast.LENGTH_LONG
            ).show()
        }
    }
    ...
    private fun refreshLabels() {
        ...
        val index =
            bubblePositions.indexOf(position).coerceAtLeast(0)

        bubblePositionValue.text =
            bubblePositionLabels[index]

        val backupFolder =
            SettingsManager.getBackupFolderUri(this)

        backupFolderValue.text =
    ...
    private fun showResolutionDialog() {
        ...
        val current =
            resolutionOptions.indexOf(
                SettingsManager.getResolutionHeight(this)
            )
        ...
        AlertDialog.Builder(this)
            ...
            .setSingleChoiceItems(labels, current) { dialog, which ->
                ...
                if (chosen == 1080 &&
                    !SettingsManager.isProUser(this)
                ) {
                    dialog.dismiss()
                    showProDialog(
                        "La résolution 1080p est réservée à la version Pro."
                    )
                } else {
                    SettingsManager.setResolutionHeight(
                        this,
                        chosen
                    )
                    refreshLabels()
    ...
    private fun showBitrateDialog() {
        ...
        val current =
            bitrateOptions.indexOf(
                SettingsManager.getBitrateMbps(this)
            )
        ...
        AlertDialog.Builder(this)
            ...
            .setSingleChoiceItems(labels, current) { dialog, which ->
                ...
                SettingsManager.setBitrateMbps(
                    this,
                    bitrateOptions[which]
                )
    ...
    private fun showFrameRateDialog() {
        ...
        val current =
            frameRateOptions.indexOf(
                SettingsManager.getFrameRate(this)
            )
        ...
        AlertDialog.Builder(this)
            ...
            .setSingleChoiceItems(labels, current) { dialog, which ->
                ...
                SettingsManager.setFrameRate(
                    this,
                    frameRateOptions[which]
                )
    ...
    private fun showCountdownDialog() {
        ...
        val current =
            countdownOptions.indexOf(
                SettingsManager.getCountdownSeconds(this)
            )
        ...
        AlertDialog.Builder(this)
            ...
            .setSingleChoiceItems(labels, current) { dialog, which ->
                ...
                if (chosen == 10 &&
                    ...
                    dialog.dismiss()
                    showProDialog(
                        "Le compte à rebours de 10s est réservé à la version Pro."
                    )
                } else {
                    SettingsManager.setCountdownSeconds(
                        this,
                        chosen
                    )
                    refreshLabels()
    ...
    private fun showBubblePositionDialog() {
        val current =
            bubblePositions.indexOf(
                SettingsManager.getBubblePosition(this)
            ).coerceAtLeast(0)

        AlertDialog.Builder(this)
            .setTitle("Position du bouton magique")
            .setSingleChoiceItems(
                bubblePositionLabels.toTypedArray(),
                current
            ) { dialog, which ->

                SettingsManager.setBubblePosition(
                    this,
                    bubblePositions[which]
                )
    ...
    override fun onPause() {
        ...
        val text =
            watermarkTextInput.text.toString().trim()

        if (text.isNotEmpty()) {
            SettingsManager.setWatermarkText(this, text)
        }
    }
}