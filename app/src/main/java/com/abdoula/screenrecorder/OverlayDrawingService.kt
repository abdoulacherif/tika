...
class OverlayDrawingService : Service() {
    ...
    private fun gradientRoundedRect(
        colors: IntArray,
        radius: Float
    ): GradientDrawable {
        return GradientDrawable(
    ...
    private fun makeBubbleInteractive(
        view: View,
    ...
    private fun minimizeBubble() {
        ...
        bubbleView?.apply {
            text = "▸"
            textSize = 16f
            background = gradientRoundedRect(
                intArrayOf(
                    Color.parseColor("#997C4DFF"),
                    Color.parseColor("#995E35B1")
                ),
                24f
            )
        }
    ...
    private fun repositionPanelIfVisible(
        forceShow: Boolean = false
    ) {
        if (!panelVisible && !forceShow)
            return
    ...
    private fun buildPanel() {
        panelView = LinearLayout(this).apply {
            ...
            background = gradientRoundedRect(
                intArrayOf(
                    Color.parseColor("#EE1E1E1E"),
                    Color.parseColor("#EE2A1B45")
                ),
                22f
            )
        ...
        row1.addView(
            makeIconButton(
                ...
                setTool(ShapeTool.ARROW)
            }
        )

        row1.addView(
            makeIconButton(
                R.drawable.ic_circle_tool,
                R.drawable.bg_round_green
            ) {
                setTool(ShapeTool.CIRCLE)
            }
        )

        row1.addView(
            makeIconButton(
                R.drawable.ic_rect_tool,
                R.drawable.bg_round_orange
            ) {
                setTool(ShapeTool.RECTANGLE)
            }
        )
        ...
        row1.addView(
            makeIconButton(
                ...
                setTool(ShapeTool.TEXT)
            }
        )

        panelView?.addView(row1)

        // ---------- Outil confidentialité ----------
    ...
    private fun updateDrawingTouchability() {
        val params =
            drawingView?.layoutParams as? WindowManager.LayoutParams
                ?: return
